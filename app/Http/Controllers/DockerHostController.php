...
{
    public function index(AgentLifecycle $lifecycle, AgentCompatibility $compatibility): Response
    {
        return Inertia::render('DockerHosts/Index', [
            'hosts' => DockerHost::withCount(['volumes' => fn ($query) => $query->where('exists', true)])
                ->orderBy('id')->get()->map(fn (DockerHost $host): array => [
                    ...
                    'volume_count' => $host->isLocal() && DeploymentMode::isOrchestrator() ? 0 : $host->volumes_count,
