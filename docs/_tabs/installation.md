...
Compatible agents provide inventory discovery and execute standalone backups and restores. Backup forms select a source host; restore forms select a target host. Shared network destinations support A-to-B restore into a new volume. A host-local destination belongs to its owner and can currently be restored only on that same host. Distributed groups, remote Docker-label reconciliation and archive relaying remain under development. The existing Volumes, Stacks and Dashboard inventory pages remain local-only; job/restore forms and Docker hosts expose agent inventories.
...
2. Create a standalone backup job, select its Docker host, then select a volume or allowed host path on that host. Network destinations are shared; filesystem/Docker-volume destinations must be owned by the same host as the job.
3. Trigger the job or let the central scheduler queue it. Offline hosts may retain queued work until the agent returns. The agent launches Offen against its own configured Docker daemon and reports the sanitized result after container cleanup.
...
2. Enter maintenance for the agent. This persists a new maintenance nonce and blocks new work admission. Already accepted groups and safety-backup children may complete; stale-work recovery and cleanup remain available.
3. Wait for a fresh agent heartbeat acknowledging that nonce and reporting no active operations. The central ledger must also contain no running operations, active groups or pending container cleanup for the host. Queued work remains queued and does not prevent readiness.
