...
- A Docker TCP endpoint, such as a socket proxy for the local engine, can be configured through `DOCKER_HOST`; multi-host execution uses agents rather than a central list of remote Docker endpoints. Standalone remote backups and restores are available; distributed groups, remote label reconciliation and host-local archive relaying remain pending.
...
- The backup container engine is reusable and agents run the existing backup/restore safety pipeline in isolated private runtimes. Multi-host group orchestration and archive relaying are still pending.
...
- Add distributed groups, remote label reconciliation and relay for archives held on another host.
