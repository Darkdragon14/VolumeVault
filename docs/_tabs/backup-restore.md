...
New jobs default to the simple include mode; existing jobs keep exclude mode. Both modes are also available through the API via `backup_filter_mode` (`include` or `exclude`), `backup_include_paths`, and `backup_exclude_regexp`.
